...
const double sigma0 = 20.3* 0.3894; // gev^-2
...
double sigma_dipolo(double r, double x, std::string N_method)
{
    if (N_method == "bCGC") {
        ...
        double sigma = 2.0 * integrate_simpson(integrand, 0.0, 2.0, 200); // integrar em b de 0 a 2 GeV^-1
        return sigma;
    }
...
// -------------- psf integrada ---------------------
double T_integrated(double bmax, int Nb = 200)
{
    auto fb = [&](double b) {
        return T(b) * 2.0 * M_PI * b; // incluir fator 2πb do jacobiano
    };
    return integrate_simpson(fb, 0.0, bmax, Nb);
}
